...
SRC_TRAIN_DATA = "./ID_seq/train.gw"          # 源语言输入文件。
TRG_TRAIN_DATA = "./ID_seq/train.xh"          # 目标语言输入文件。
# CHECKPOINT_PATH = "./attention_ckpt"   # checkpoint保存路径。
MODEL_SAVE_PATH = "./attention"
...
class NMTModel(object):
    ...
    def __init__(self):
        ...
        self.src_embedding = tf.Variable(tf.random_uniform([SRC_VOCAB_SIZE, HIDDEN_SIZE], -1.0, 1.0),trainable=False,name="src_emb")
        # self.trg_embedding = tf.get_variable("trg_emb", [TRG_VOCAB_SIZE, HIDDEN_SIZE])
        self.trg_embedding = tf.Variable(tf.random_uniform([TRG_VOCAB_SIZE, HIDDEN_SIZE], -1.0, 1.0), trainable=False,name="trg_emb")
...
def run_epoch(session, cost_op, train_op, saver, step,global_step):
    # 训练一个epoch。
    ...
    while True:
        try:
            ...
            if step % 100 == 0:
                print("After %d steps, per token cost is %.3f" % (global_step+step, cost))
            # 每200步保存一个checkpoint。
            if step % 200 == 0:
                # saver.save(session, CHECKPOINT_PATH, global_step=step)
                # 自己更改的
                saver.save(session, os.path.join(MODEL_SAVE_PATH, MODEL_NAME), global_step=global_step+step)
            step += 1
...
def main():
    ...
    saver_gw=tf.train.Saver({"embedding":train_model.src_embedding})
    saver_xh = tf.train.Saver({"embedding": train_model.trg_embedding})
    step = 0
    with tf.Session() as sess:
        tf.global_variables_initializer().run()

        ckpt_gw= tf.train.get_checkpoint_state("./gwmodel")
        if (ckpt_gw and ckpt_gw.model_checkpoint_path):
            saver_gw.restore(sess, ckpt_gw.model_checkpoint_path)
            a=sess.run(train_model.src_embedding)
        ckpt_xh = tf.train.get_checkpoint_state("./xhmodel")
        if (ckpt_xh and ckpt_xh.model_checkpoint_path):
            saver_xh.restore(sess, ckpt_xh.model_checkpoint_path)
            b=sess.run(train_model.trg_embedding)
        ...
        for i in range(NUM_EPOCH):
            ...
            step = run_epoch(sess, cost_op, train_op, saver, step,global_step)
