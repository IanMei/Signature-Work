...
# 词汇表文件
SRC_VOCAB ="./vocab/gw.vocab"
TRG_VOCAB = "./vocab/xh.vocab"
...
class NMTModel(object):
    ...
    def __init__(self):
        # 定义编码器和解码器所使用的LSTM结构。
        self.enc_cell_fw = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(HIDDEN_SIZE)
        self.enc_cell_bw = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(HIDDEN_SIZE)
        self.dec_cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell(
            [tf.compat.v1.nn.rnn_cell.BasicLSTMCell(HIDDEN_SIZE)
             for _ in range(DECODER_LAYERS)])

        # 为源语言和目标语言分别定义词向量。
        # self.src_embedding = tf.get_variable("src_emb", [SRC_VOCAB_SIZE, HIDDEN_SIZE])
        self.src_embedding = tf.Variable(tf.compat.v1.random_uniform([SRC_VOCAB_SIZE, HIDDEN_SIZE], -1.0, 1.0), trainable=False,name="src_emb")
        # self.trg_embedding = tf.get_variable("trg_emb", [TRG_VOCAB_SIZE, HIDDEN_SIZE])
        self.trg_embedding = tf.Variable(tf.compat.v1.random_uniform([TRG_VOCAB_SIZE, HIDDEN_SIZE], -1.0, 1.0), trainable=False,
                                         name="trg_emb")
        # 定义softmax层的变量
        if SHARE_EMB_AND_SOFTMAX:
            self.softmax_weight = tf.transpose(self.trg_embedding)
        else:
            self.softmax_weight = tf.compat.v1.get_variable(
                "weight", [HIDDEN_SIZE, TRG_VOCAB_SIZE])
        self.softmax_bias = tf.compat.v1.get_variable(
...
def main():
    ...
    # 定义个测试句子。
    test_en_text = "年 三 岁 而 宣 帝 崩 ， 元 帝 即 位  ， 帝 为 太 子  。 <eos>"
    print(test_en_text)
    ...
    a=sess.run(model.src_embedding)
    print(a[50][100])
    # 读取翻译结果。
    output_ids = sess.run(output_op)
    print(output_ids)
    output_ids=numpy.delete(output_ids,[0,len(output_ids)-1])
